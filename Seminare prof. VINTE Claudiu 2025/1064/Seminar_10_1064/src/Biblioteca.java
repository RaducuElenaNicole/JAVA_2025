...
//            private int an; // an aparitie (int)
            sb.append("cota varchar(16) primary key,");
            sb.append("titlu varchar(64),");
            sb.append("autori varchar(64),");
            sb.append("an int");
            sb.append(" )");
    ...
    private static void insertCarte(Carte c) {
        ...
        try {
            ...
            sb.append(" values(");
            sb.append("'");sb.append(c.getCota());sb.append("'");sb.append(",");
            sb.append("'");sb.append(c.getTitlu()); sb.append("'");sb.append(",");
            sb.append("'");sb.append(c.getAutori()); sb.append("'");sb.append(",");
            sb.append(c.getAn());sb.append(")");
            sqlStmt.execute(sb.toString());
            // String query = "insert into " + tableName + " values ("+
            System.out.println("Carte inserata cu succes!");
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
    }
    ...
    private static void updateCarte(Carte c) {
        ...
        try {
            Statement sqlStmt = conn.createStatement();
            String querySQL = "update "+tableName + " set cota='Cota-0002'"+
                    " where cota='"+c.getCota()+"'";

            sqlStmt.execute(querySQL);
    ...
    private static void deleteCarte(Carte c) {
        ...
        try {
            Statement sqlStmt = conn.createStatement();
            String querySQL = "delete from "+tableName +" where an="+c.getAn();
            sqlStmt.execute(querySQL);
