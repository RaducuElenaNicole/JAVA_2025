...
class ClientTCP {
    public static void main(String[] args) throws IOException {
        try(Socket socket = new Socket("localhost", 8080)){
            ...
            while(true){
                String aventura = new Scanner(System.in).nextLine();
                bw.write(aventura);
//                if(aventura.equalsIgnoreCase("Aici nu este client, este IGNORE")){
//                    br.close();
//                    bw.close();
//                    break;
//                }else{
...
public class ProgramPrincipal_TCP{
    public static void main(String[] args) throws IOException {
        // citire din JSON
    ...
    public static void clientHandler(Socket socket) throws IOException {
        BufferedReader br = new BufferedReader(new InputStreamReader(socket.getInputStream()));
        BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(socket.getOutputStream()));

        while(true) {
            String denumirePrimita = br.readLine();
            bw.write(denumirePrimita);
            bw.flush();
        }
